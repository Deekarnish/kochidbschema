...
BEGIN
    ...
    WITH basedata AS (
	    ...
	    total_available_units AS (
			select count(1)/2 units_available from (
				select * from ( 
					select tl ,bd.node_id ,state, count(1) over (partition by tl, bd.node_id) cnt from basedata bd
					) a
				where state not in ('ERROR','ON_HOLD','MAINTENANCE')  and cnt <=10 -- 
			) b
		),
    ...
    SELECT  coalesce(((cardinality($3) - array_length(un.unavailable_nodes, 1))::FLOAT / cardinality($3) * 100),100) AS availability_percentage
    FROM date_series ds
    LEFT JOIN total_available_units ta ON ds.generated_date = ta.tl
    LEFT JOIN unavailable_nodes un ON ds.generated_date = un.tl;
...
--
-- Name: calculate_throughput_report_test(date, date, integer[]); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.calculate_throughput_report_test(start_date date, end_date date, node_id integer[]) RETURNS TABLE(date_ date, node integer, throughput_percentage double precision)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
    WITH total_count AS (
        select "CreatedOn"::date as request_date,n."NodeId",count(1) total_count from "Request" r 
        LEFT JOIN 
                    "Node" n ON r."NodeIdentifier" = n."NodeIdentifier"
                WHERE 
                    r."CreatedOn"::date BETWEEN start_date AND end_date
                    AND n."IsDeleted" = FALSE
                    AND n."NodeId" = ANY (node_id)
		group by "CreatedOn"::date ,n."NodeId" ) ,
    transactions_within_146s AS (
        select "CreatedOn"::date as request_date,n."NodeId",count(1) count_within_146s from "Request" r 
        LEFT JOIN 
                    "Node" n ON r."NodeIdentifier" = n."NodeIdentifier"
                WHERE 
                    r."CreatedOn"::date BETWEEN start_date AND end_date
                    AND n."IsDeleted" = FALSE
                    AND n."NodeId" = ANY (node_id)
                    and EXTRACT(epoch FROM (r."LastUpdatedTime"::timestamp - r."CreatedOn"::timestamp)) <=146 
           group by "CreatedOn"::date ,n."NodeId"
    )
    SELECT 
        tc.request_date AS date_, 
        tc."NodeId" AS node, 
        sum(tw.count_within_146s::FLOAT / tc.total_count) * 100 AS throughput_percentage
    FROM 
        total_count tc
    JOIN 
        transactions_within_146s tw ON tc.request_date = tw.request_date AND tc."NodeId" = tw."NodeId"
...
RETURN QUERY  
	...
	active as (
		select tl,count(1) active_count from (
			...
			where state in ('ACTIVE','ENGAGED') and cnt=1
		) b
		group by tl
	),
	error as (
	...
	inactive as (
		select tl,count(1) inactive_count from (
			select tl ,name ,state, count(1) over (partition by tl,name) cnt from basedata
			where state in ('INACTIVE','DUT_CHANGING','VERIFICATION_COMPLETE', 'VERIFICATION_FAILED', 'MAINTENANCE')
			) a
		group by tl		
	),
	timeline as (select generate_series($1,
	$2,'1 day'::interval)::date tl
	)
	select a.tl::date time_ ,coalesce(active_count,0) , coalesce(error_count,0) error_count  ,
	coalesce(onhold_count,0) onhold_count ,  coalesce(inactive_count,0) inactive_count 
    from 
    	timeline a
    	left join active ac on a.tl =ac.tl 
...
--
-- Name: graphtest(date, date); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.graphtest(startdate date, enddate date) RETURNS TABLE(time_ date, active_count_ bigint, error_count_ bigint, onhold_count_ bigint, inactive_count_ bigint)
    LANGUAGE plpgsql
    AS $_$
BEGIN
RETURN QUERY     
...
--
-- Name: sum_of_all_test(date, date, integer[]); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.sum_of_all_test(start_date date, end_date date, node_id integer[]) RETURNS TABLE(total_throughput_percentage double precision, total_mean_avg_time_percentage double precision, total_availability_percentage double precision)
    LANGUAGE plpgsql
    AS $$
DECLARE
    total_throughput_percentage_val double precision := 0;
...
BEGIN
    -- Calculate total throughput percentage
  /*  FOR total_throughput_percentage_val IN
        SELECT coalesce(SUM(throughput_percentage),0) FROM public.calculate_throughput_report(start_date, end_date, node_id)
    LOOP
        -- Calculate total mean average time percentage
        FOR total_mean_avg_time_percentage_val IN
            SELECT coalesce(SUM(mean_avg_time_percentage),0) FROM public.mean_avg_transaction_time_report(start_date, end_date, node_id)
        LOOP
            -- Calculate total availability percentage
            SELECT coalesce(SUM(availability_percentage),0) INTO total_availability_percentage_val FROM public.availability_report(start_date, end_date, node_id);
   
                     
            RETURN QUERY SELECT total_throughput_percentage_val, total_mean_avg_time_percentage_val, total_availability_percentage_val;
        END LOOP;
    END LOOP; */
	SELECT coalesce(AVG(availability_percentage),0) INTO total_availability_percentage_val FROM public.availability_report(start_date, end_date, node_id);
	SELECT coalesce(AVG(throughput_percentage),0) into total_throughput_percentage_val FROM public.calculate_throughput_report(start_date, end_date, node_id);
	SELECT coalesce(AVG(mean_avg_time_percentage),0) into total_mean_avg_time_percentage_val FROM public.mean_avg_transaction_time_report(start_date, end_date, node_id);
	RETURN QUERY SELECT total_throughput_percentage_val, total_mean_avg_time_percentage_val, total_availability_percentage_val;
END;
$$;
...
--
-- Name: test_availability_report(date, date, text[]); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.test_availability_report(start_date date, end_date date, node_name text[]) RETURNS TABLE(date date, unavailable_nodes text[], active_nodes double precision, available_nodes double precision, availability_percentage double precision)
    LANGUAGE plpgsql
    AS $_$
BEGIN
    RETURN QUERY
    WITH basedata AS (
	    SELECT DISTINCT timeline_::date AS tl, name, state FROM testdata t
	    ),
	    date_series AS (
    		SELECT generate_series($1, $2, '1 day'::interval)::date AS generated_date
		),
	    total_available_units AS (
			select tl,count(1) units_available from (
				select * from ( 
					select tl ,name ,state, count(1) over (partition by tl,name) cnt from basedata
					) a
				where state='ACTIVE' and cnt=1 AND tl BETWEEN $1 AND $2 AND name = ANY ($3)
			) b
			group by tl
		),
	    unavailable_nodes AS (
	        SELECT tl, ARRAY_AGG(DISTINCT name) AS unavailable_nodes
	        FROM basedata
	        WHERE state <> 'ACTIVE' AND tl BETWEEN $1 AND $2 AND name = ANY ($3)
	        GROUP BY tl
	    )
    SELECT ds.generated_date AS date,
           COALESCE(un.unavailable_nodes, '{}') AS unavailable_nodes,
           units_available::FLOAT as active_nodes,
           (cardinality($3) - array_length(un.unavailable_nodes, 1))::FLOAT as available_nodes,
           ((cardinality($3) - array_length(un.unavailable_nodes, 1))::FLOAT / cardinality($3) * 100) AS availability_percentage
    FROM date_series ds
    LEFT JOIN total_available_units ta ON ds.generated_date = ta.tl
    LEFT JOIN unavailable_nodes un ON ds.generated_date = un.tl;
...
--
-- Name: test_calculate_throughput_report(date, date, text[]); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.test_calculate_throughput_report(start_date date, end_date date, node_name text[]) RETURNS TABLE(date_ date, node text, count_within_146s bigint, total_count bigint, throughput_percentage double precision)
    LANGUAGE plpgsql
    AS $_$
BEGIN
RETURN QUERY
WITH basedata AS (
	SELECT distinct timeline_::date tl, name, state from testdata t
	),
    total_count as (
    	select tl, name, count(1) total_count from (
    		select * from (
    			select tl, name, state, count(1) over (partition by tl, name) cnt from basedata bd
    			) a
    		where state = 'ACTIVE' and cnt=1 and  tl BETWEEN $1 AND $2 and name = ANY ($3)
    	) b
    	GROUP BY tl, name
    ...
    transactions_within_146s as (
        select tl, name, count(1) count_within_146s from (
        	select * from (
        		select tl , name , state, count(1) over (partition by tl, name) cnt from  basedata
        		) a
        	where state = 'ACTIVE' and cnt=1 and  tl BETWEEN $1 AND $2 and name = ANY ($3)
        	and EXTRACT(SECOND FROM tl::timestamptz) <= 146
        ) b
        GROUP BY tl, name
    )
    SELECT tc.tl date_, tc.name node, 
    tw.count_within_146s,
    tc.total_count,
           (tw.count_within_146s::FLOAT / tc.total_count) * 100 AS throughput_percentage
    FROM total_count tc
    JOIN transactions_within_146s tw ON tc.tl = tw.tl
                                        AND tc.name = tw.name;
END;
$_$;
...
ALTER TABLE public.nsh1 ALTER COLUMN "NodeStatusHistoryId" ADD GENERATED BY DEFAULT AS IDENTITY (
...
--
-- Name: nsh2_NodeStatusHistoryId_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public."NodeStatusHistory" ALTER COLUMN "NodeStatusHistoryId" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public."nsh2_NodeStatusHistoryId_seq"
    START WITH 0
    INCREMENT BY 1
    MINVALUE 0
    NO MAXVALUE
...
--
-- Name: testdata; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.testdata (
    timeline_ timestamp with time zone,
    name character varying(100),
...
--
-- Name: nsh1 NodeStatusHistory_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: NodeStatusHistory NodeStatusHistory_pkey2; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public."NodeStatusHistory"
    ADD CONSTRAINT "NodeStatusHistory_pkey2" PRIMARY KEY ("NodeStatusHistoryId");
...
CREATE INDEX "NodeStatusHistory_NodeId_idx" ON public.nsh1 USING btree ("NodeId");
...
CREATE INDEX "NodeStatusHistory_StatusId_idx" ON public.nsh1 USING btree ("StatusId");
